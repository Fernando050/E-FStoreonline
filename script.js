...
function updateGallery(category, showAll = false) {
  ...
  if (category !== 'tout' && count > LIMIT_INITIAL && !showAll) {
...
// 6. SISTÈM PANYE ACHA INTELLIGENT
let panier = [];
...
function envoyerCommandeWhatsApp() {
  ...
  let message = `Bonjour *E&F Store Online*, je souhaite valider ma commande :%0A%0A`;
  
  let total = 0;
  panier.forEach((item, index) => {
    total += item.prixValue;
    message += `📦 ${index + 1}. *${item.nom}* - ${item.prixText}%0A`;
  });

  message += `%0A━━━━━━━━━━━━━━━%0A`;
  message += `💰 *TOTAL À PAYER : ${total.toLocaleString()} HTG*%0A`;
  message += `━━━━━━━━━━━━━━━%0A%0A`;
